...
if (isset($_GET['dari']) && isset($_GET['ke'])) {
    ...
    // Query untuk mengambil data transaksi masuk berdasarkan periode tanggal
    $query = "SELECT waktu, nominal_produk, harga_produk, keuntungan, no_tujuan FROM transaksi_masuk WHERE DATE(waktu) BETWEEN '$dari' AND '$ke' ORDER BY waktu ASC";
    $filename = 'Lap_Trx_Masuk_' . $dari . '_Ke_' . $ke . '.pdf'; // Nama file output

} else {
    // Jika parameter dari dan ke tanggal tidak tersedia, tampilkan semua data
    $query = "SELECT waktu, nominal_produk, harga_produk, keuntungan, no_tujuan FROM transaksi_masuk ORDER BY waktu ASC";
    $filename = 'Lap_Trx_Masuk_All.pdf'; // Nama file output
}
